# -*- coding: utf-8 -*-
#-------------------------------------------------------------------------------
# Name:     4calcAUROC_LSM         prueba compleja con repeticiones y p. corte
#
...
#   Ahora, ambos valores LSI se eliminan con la selección de un límite inferior "limInf"
...
#
# Se ha modificado el cálculo de AUROC para cada intervalo mediante el cálculo de
#   McClish (1989) para el parcial AUC estandarizado (pAUC)

# NO se crean ya las tablas parciales, sólo la final: mucho más rápido!
#
...
#
# Created:     18/07/2019, revisado 27/08/19
...
fR = "MFR7"   # OJO! indicar si es ClusterFR CFR o General FR1 o FR2
#fR = "EasyROC"   # para pruebas
limInf = [0, 0, 0]      # con limInf = 0 el cálculo es completo. Filtra valores LSI
#limInf = [356, 356, 356]    # con limInf > 0 el cálculo es parcial. Ya no parece necesario
lInt = 2  # longitud de cada intervalo. Para mayor exactitud --> EL MENOR POSIBLE!
...
elif fR == "MFR7":     # análisis cluster + regresión
    lSup = 410 # limite intervalo superior, redondeado por encima el valor + alto de LSI
    prov = ["MFR7_CS", "MFR7_VL", "MFR7_AL"]    # lista de mapas provinciales LSM creados por AHP
    if not 0 in limInf:
        limInf = [71, 63, 57]
        #limInf = [80]
    #prov = ["MFR_VL"]
...
try:
    ...
    #for lm in ["log", ""]:
        ...
        if not arcpy.Exists (resROCt+lm + str(limInf)):
            ...
            for g in ("AUC", "pAUCst", "Y_TPR_SEN", "TNR_SPE", "X_FPR"):
        ...
        for pCorte in interv:
            ...
            # ojo, ya NO se crean las tablas parciales
...
##                    arcpy.DeleteRows_management(resROCp+lm + "_" + str(pCorte))  # vacía el contenido
            ...
            for i in pCorteL[1:]:  # empezando en el segundo
                ...
                if i <= pCorte and h < 1:    # tramo final deshechado
                    ...
                    acF = fSum0 + tSum1
                    nrepF = nrep + fnC + tnC
                    #print "final", i, pCorte, h, acF, nrepF
                ...
                auc = (fprX[k] - fprX[k-1]) * (senY[k] + senY[k-1]) * 0.5
                if auc >= 0:
                    AUC.append(auc)

                sumAUC = 0   # suma de la lista AUC para obtener AUROC
                sumAUC = np.sum(AUC)   # mejor

                # calculando AUC parcial estandarizado (según McClish, 1989)
                mn = fprX[k]**2    # valor mínimo del área
                Mx = fprX[k]        # valor máximo del área
                if sumAUC > 0:  #evita indeterminación en la fórmula
                    pAUC = 0.5 * (1 + (sumAUC - mn/2)/(Mx-mn/2))
                else:
                    pAUC = 0.5  # valor mínimo

                # print "Num:", nrep, " Num acum:", ac,  " FPR (x): {0:.3f};".format(fpr), \
                # " TPR (y): {0:.3f};".format(tpr), " auroc: {0:.3f}".format(auc)

                # se graba hasta el penúltimo intervalo para que cuadren bien los datos
                # acumulados. De otra forma, el último intervalo se calcula mal
                # Realmente, no está claro por qué, pero funciona
                if datLSI[-2] == pCorte:
                    ...
                    row.pAUCst = pAUC
                    row.X_FPR = fprX[k]
                    row.Y_TPR_SEN = senY[k]
                    row.TNR_SPE = 1-fprX[k]
                ...
                k += 1
                z += 1
                #print "nrep:", i, nrep, nrepT, ac, acT
                # fin de bucle datos -----------------------
...
##
##            # calculando AUC parcial estandarizado (según McClish, 1989)
##            mn = fprC**2    # valor mínimo del área
##            Mx = fprC        # valor máximo del área
##            if sumAUC > 0:  #evita indeterminación en la fórmula
##                pAUC = 0.5 * (1 + (sumAUC - mn/2)/(Mx-mn/2))
##            else:
##                pAUC = 0.5  # valor mínimo

            print "pCorte:", pCorte, "  pAUC:", pAUC, "Ac: ", acT, "AcTot:", acF
            #print ac, nrep, nrepT, nrepF
            print

            #if  k == 9:   sys.exit()

            # Se añade el intervalo final así pora que cuadre los datos acumulados
            if datLSI[-1] == pCorte:
                rows = arcpy.InsertCursor(resROCt+lm+str(limInf[m]))
                row = rows.newRow()
                row.P_Corte = pCorte
        ...
        # GRABANDO PUNTOS DE CORTE TXT (solo para la lista de valores completa)
            ...
            for w in wOC:
                ...
                # como pCorteL incluye lSup, y sen no, hay que sumar 1 a pCorteL
